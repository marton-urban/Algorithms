...
def not_allowed(digit, digit_number, number_of_occ, time_it):
    all_digits[digit_number] = digit
    ...
    if digit_number_rem == 1:
        ...
        if digit == all_digits[digit_number - 1] or occurences.count(digit) == number_of_occ:
            return True
    elif digit_number_rem == 0:
        if digit == all_digits[digit_number - 1] or digit == all_digits[digit_number - 2] or occurences.count(
                digit) == number_of_occ:
            return True
        if digit_number == 3:
...
# why does number_of_digits decrease to 0, but then automatically changes to 1???
def generate_all_digits(number_of_digits, number_of_occ, time_it):
    digit_number = + 1
    if number_of_digits > 0:
        for digit in range(1, 10):
            if not_allowed(digit, digit_number, number_of_occ, time_it):
                continue
